...
function sanitize(row) {
  ...
  for (const [k, v] of Object.entries(config)) {
    ...
    maskedConfig[k] = sensitive.some(s => k.toLowerCase().includes(s)) && v ? "••••••••" : v;
  }
  return { ...rest, config: maskedConfig, hasData: !!cached_data, webhookUrl: row.webhook_token ? `/api/webhook/${row.webhook_token}` : null };
}

// ── PUBLIC: webhook push (no auth needed) ─────────────────────────────────────
...
router.get("/sources", (req, res) => {
  res.json(getAllSources(req.userId).map(sanitize));
});
...
router.post("/sources", async (req, res) => {
  ...
  const obj = { name, type, url: url ?? null, headers: config, db_type: db_type ?? null, db_query: db_query ?? null, user_id: req.userId, is_global: 0 };
  if (type === "json") { obj.cached_data = JSON.stringify(data); obj.last_fetched = now; }
  if (type === "webhook") { obj.webhook_token = crypto.randomBytes(24).toString("hex"); }
...
router.post("/upload-csv", upload.single("file"), (req, res) => {
  ...
  const now = new Date().toISOString();
  const source = upsertSource({ name, type: "csv", cached_data: JSON.stringify(data), last_fetched: now, user_id: req.userId, is_global: 0 });
